...
export default function AddTestResult() {
  ...
  const loadPatientAndHistory = async (routeValue) => {
    try {
      ...
      const realPatientId = routeValue;
  ...
  const handleDownloadExcel = async (session) => {
    try {
      ...
      rows.forEach((row, index) => {
        ...
        const statusCell = excelRow.getCell(headers.indexOf("Status") + 1);

        const status = String(row.Status || "").toUpperCase();

        const styles = {
          NORMAL: { bg: "FF92D050", fg: "FF000000" },
          HIGH: { bg: "FFFF0000", fg: "FFFFFFFF" },
          LOW: { bg: "FFFFC000", fg: "FF000000" },
          "CRITICAL HIGH": { bg: "FFC00000", fg: "FFFFFFFF" },
          "CRITICAL LOW": { bg: "FFE26B0A", fg: "FFFFFFFF" },
          POSITIVE: { bg: "FF92D050", fg: "FF000000" },
          NEGATIVE: { bg: "FFFF0000", fg: "FFFFFFFF" },
        };
        ...
        if (styles[status]) {
          statusCell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: styles[status].bg },
          };
          statusCell.font = {
            bold: true,
            color: { argb: styles[status].fg },
          };
        }
      });
      ...
      link.download = `${filePatientId}.xlsx`;
